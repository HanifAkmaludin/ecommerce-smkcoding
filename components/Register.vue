...
const submitRegister = async () => {
  ...
  await authState.register(result);
  ...
  <div class="w-full max-w-sm p-4 bg-white rounded-lg sm:p-6 md:p-8 shadow-[0_30px_60px_rgba(0,0,0,0.15)]">
    <form class="" @submit.prevent="submitRegister">
      <div class="mb-6">
        ...
        <span class="text-sm font-normal text-gray-500">Create an account to start using our application</span>
      </div>
      ...
      <div class="text-sm font-medium text-gray-500">
          Already have an Account ? <NuxtLink to="/login" class="text-blue-700 hover:underline">Sign in</NuxtLink>
      </div>
    </form>
  </div>
</template>